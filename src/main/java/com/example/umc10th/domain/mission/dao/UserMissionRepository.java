...
public interface UserMissionRepository extends JpaRepository<UserMission, Long> {
    ...
    @Query(value = "SELECT um FROM UserMission um JOIN FETCH um.mission m JOIN FETCH m.store WHERE um.user.id = :userId AND um.status = :status",
            countQuery = "SELECT count(um) FROM UserMission um WHERE um.user.id =:userId AND um.status = :status") //Checks for paging error
    Page<UserMission> findAllByUserIdAndStatus(@Param("userId") Long userId, @Param("status") MissionStatus status, Pageable pageable);


    Page<UserMission> findAllByMissionStoreId(Long storeId, Pageable pageable);
}
